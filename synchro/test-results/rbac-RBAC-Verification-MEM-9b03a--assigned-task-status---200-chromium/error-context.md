...
- Name: rbac.spec.ts >> RBAC Verification >> MEMBER Role >> MEMBER -> update assigned task status -> 200
- Location: tests\e2e\rbac.spec.ts:92:9
...
> 101 |       expect(res.status()).toBe(200);
  ...
  140 |     
  141 |     test.beforeAll(async ({ playwright }) => {
  142 |       adminContext = await playwright.request.newContext({ baseURL: 'http://localhost:3000' });
  143 |       await adminContext.post('/api/auth/login', { data: users.admin });
  144 |     });
  145 | 
  146 |     test('ADMIN -> delete task -> 200', async () => {
  147 |       const res = await adminContext.delete(`/api/workspaces/${workspaceId}/tasks/${taskCreatedByManager}`);
